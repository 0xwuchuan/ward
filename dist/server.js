...
async function readPreviewFile(target) {
    const stat = await fs.stat(target);
    if (!stat.isFile())
        throw new WardDbError('File was not found.');
...
async function codePreview(project, ref) {
    ...
    if (!path.isAbsolute(rawPath) && inBoundsCandidates.length > 1) {
        const existingCandidates = (await Promise.all(inBoundsCandidates.map(async (candidate) => {
            try {
...
export function createApp() {
    ...
    app.delete('/api/projects/:projectId', (c) => {
        if (!deleteProject(c.req.param('projectId')))
            return c.json({ detail: 'project not found' }, 404);
    ...
    app.post('/api/projects/:projectId/fix-review', async (c) => {
        ...
        try {
            ...
            return c.json(requestFixReview({
                projectId,
                commitHash,
