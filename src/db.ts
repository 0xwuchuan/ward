...
export function connect(target?: string): Sqlite {
...
function withDb<T>(db: Sqlite | undefined, fn: (conn: Sqlite) => T): T {
  if (db) return fn(db)
  const owned = connect()
  try {
    return fn(owned)
...
function validateChoice<T extends string>(field: string, value: string, choices: readonly T[]): T {
...
function normalizeFileRef(ref: Record<string, unknown>): FileRef {
  const refPath = String(ref.path ?? '').trim()
  if (!refPath) throw new WardDbError('file reference path is required')

  const normalized: FileRef = { path: refPath }
  ...
  if (
    normalized.start_line !== undefined &&
...
export function normalizeFileRefs(fileRefs?: Iterable<Record<string, unknown>> | null): FileRef[] {
  return [...(fileRefs ?? [])].map(normalizeFileRef)
}

export function parseFileRefs(values?: Iterable<string> | null): FileRef[] {
  const refs: FileRef[] = []
  for (const raw of values ?? []) {
    const value = raw.trim()
    if (!value) continue
    const match = fileRefPattern.exec(value)
    if (!match?.groups) throw new WardDbError(`invalid file reference: ${raw}`)
    const ref: Record<string, unknown> = { path: match.groups.path }
    if (match.groups.start) ref.start_line = Number(match.groups.start)
    if (match.groups.end) ref.end_line = Number(match.groups.end)
    refs.push(normalizeFileRef(ref))
  }
  return refs
}

function decodeFileRefs(value: string): FileRef[] {
  try {
    const loaded = JSON.parse(value || '[]')
    if (!Array.isArray(loaded)) return []
    return loaded.filter((ref): ref is FileRef => typeof ref === 'object' && ref !== null)
  } catch {
    return []
  }
}
...
function findingFromRow(row: FindingRow): FindingOut {
  return {
    id: row.id,
...
export function resolveProject(identifier?: string | null, cwd = process.cwd(), db?: Sqlite): ProjectOut | null {
  return withDb(db, (conn) => {
    if (identifier) {
...
export function createFinding(options: {
  ...
  return withDb(options.db, (conn) => {
    if (!getProject(options.project_id, conn)) throw new WardDbError('project not found')
    const title = options.title.trim()
    if (!title) throw new WardDbError('title is required')
    ...
    conn
      ...
      .run(
        ...
        options.project_id,
        title,
        validateChoice('severity', options.severity, severities),
        JSON.stringify(refs),
        (options.category ?? '').trim(),
        (options.description ?? '').trim(),
        (options.impact ?? '').trim(),
        (options.recommendation ?? '').trim(),
        validateChoice('source', options.source ?? 'human', sources),
        validateChoice('status', options.status ?? 'draft', statuses),
        timestamp,
        timestamp,
      )
    const created = getFinding(findingId, conn)
    if (!created) throw new WardDbError('finding not found')
    return created
  })
}

export function getFinding(findingId: string, db?: Sqlite): FindingOut | null {
  return withDb(db, (conn) => {
    const row = conn.prepare('SELECT * FROM findings WHERE id = ?').get(findingId) as FindingRow | undefined
  ...
  return withDb(db, (conn) => {
    ...
    for (const [key, rawValue] of Object.entries(values)) {
      ...
      if (key === 'title') {
        ...
        if (!value) throw new WardDbError('title is required')
        ...
        if (!Array.isArray(rawValue)) throw new WardDbError('file_refs must be a list')
        value = JSON.stringify(normalizeFileRefs(rawValue as Record<string, unknown>[]))
      } else {
        value = String(rawValue).trim()
      }
...
export function deleteFinding(findingId: string, db?: Sqlite): boolean {
  return withDb(db, (conn) => {
